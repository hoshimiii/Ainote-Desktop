...
The assistant tool execution path SHALL report a write tool as successful only after its planned formal kanban commands have executed successfully. In structured workflow mode, the system MUST invoke assistant tools through an explicit registered tool-call boundary before formal command execution.
...
- **THEN** the system MUST invoke a registered assistant tool, execute the formal commands returned by that tool, and return a handled success response with affected state available in the saved snapshot
...
- **THEN** the planner result MUST include internal trace metadata identifying the invoked tool and whether it produced executable commands
