...
export async function cloudLogin(
  ...
  try {
    ...
    // Step 2: POST credentials to Auth.js callback
    const callbackRes = await fetch(`${base}/api/auth/callback/credentials`, {
      ...
      redirect: 'manual', // Auth.js redirects on success
    })

    // Capture session cookies from callback response
    const sessionCookies = extractSetCookies(callbackRes)
    const allCookies = mergeCookies(csrfCookies, sessionCookies)

    // Step 3: Verify the session by calling /api/auth/session
    ...
    // Include any new cookies from the session call
...
/** Extract all Set-Cookie headers and format as a Cookie header value */
...
/** Merge two cookie strings, newer values overriding older ones */
...
/** Check if cloud is configured and session is valid */
...
/** Logout from cloud (clear stored session) */
export function cloudLogout(): void {
  clearCloudSession()
}

/**
 * Get all records pending sync (localUpdatedAt > cloudUpdatedAt).
 */
...
/**
 * Mark records as synced after successful cloud push.
 */
...
/**
 * Get full snapshot of local Zustand store state for cloud backup.
 */
...
/**
 * Apply cloud state snapshot to local SQLite store.
 * Used during pull/restore operations.
 */
...
export async function pushToCloud(): Promise<SyncResult> {
  ...
  try {
    ...
    // Transform desktop format → AiNote_online cloud format
    ...
    // Update lastSyncTime on success (task 2.3)
...
/**
 * Pull cloud snapshot to local store.
 * GETs /api/sync and applies if cloud is newer.
 */
...
export function getSyncStatus(): {
  ...
  // Read lastSyncTime from settings KV
