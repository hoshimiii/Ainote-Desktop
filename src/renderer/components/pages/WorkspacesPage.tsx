        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <DeleteDialog
                ...
                trigger={
                  <button
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-red-500 hover:bg-red-500/10 transition-colors"
                    title="重置本地数据"
                  >
              ...
              />
              <button
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-on-surface-variant hover:bg-surface-container-high transition-colors"
                onClick={logout}
                title="登出"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {workspaces.map((ws) => (
              <Card
                key={ws.id}
                className="cursor-pointer group hover:bg-surface-container-high transition-colors"
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                    <RenameDialog
                      initialName={ws.name}
                      onConfirm={(newName) => renameWorkSpace(ws.id, newName)}
                    />
                    <DeleteDialog
                      title="删除工作区"
                      description={`确定要删除 "${ws.name}" 吗？`}
