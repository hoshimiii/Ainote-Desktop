  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    switch (type) {
      case 'mission': {
        ...
        if (oldIdx !== -1 && newIdx !== -1) reorderMissions(activeWorkSpaceId, arrayMove(ids, oldIdx, newIdx))
        break
      }
      case 'board':
      case 'board-card': {
        ...
        const oldIdx = ids.indexOf(activeIdStr)
        const newIdx = ids.indexOf(overIdStr)
        if (oldIdx !== -1 && newIdx !== -1) reorderBoards(currentMissionId, arrayMove(ids, oldIdx, newIdx))
        break
      }
      case 'task': {
        ...
        if (oldIdx !== -1 && newIdx !== -1) reorderTasks(boardId, arrayMove(board.taskIds, oldIdx, newIdx))
        break
      }
      case 'subtask': {
        ...
        const oldIdx = ids.indexOf(activeIdStr)
        const newIdx = ids.indexOf(overIdStr)
        if (oldIdx !== -1 && newIdx !== -1) reorderSubtasks(taskId, arrayMove(ids, oldIdx, newIdx))
        break
      }
      case 'note': {
        if (!currentMissionId) break
        ...
        const oldIdx = ids.indexOf(activeIdStr)
        const newIdx = ids.indexOf(overIdStr)
        if (oldIdx !== -1 && newIdx !== -1) reorderNotes(currentMissionId, arrayMove(ids, oldIdx, newIdx))
        break
      }
      case 'block': {
        ...
        const oldIdx = ids.indexOf(activeIdStr)
        const newIdx = ids.indexOf(overIdStr)
        if (oldIdx !== -1 && newIdx !== -1) reorderBlocks(currentNoteId, arrayMove(ids, oldIdx, newIdx))
        break
      }
    }
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      <WindowTitlebar
        ...
        actions={
          <>
            <button className="p-1 rounded hover:bg-surface-container-high" onClick={() => setSyncPanelOpen(true)} title="云端同步">
              <span className="material-symbols-outlined text-sm text-on-surface-variant">cloud_sync</span>
            </button>
            <button className="p-1 rounded hover:bg-surface-container-high" onClick={logout} title="登出">
      ...
      {rehydrationError && (
        <div className="mx-4 mt-3 flex items-start justify-between gap-3 rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-red-700">
          ...
          <button className="flex-shrink-0 rounded-lg px-2 py-1 text-xs text-red-700 hover:bg-red-500/10" onClick={clearRehydrationError}>关闭</button>
        </div>
      )}
      ...
      {transientRecoveryMessage && (
        <div className="mx-4 mt-3 flex items-start justify-between gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-amber-800">
          ...
          <button className="flex-shrink-0 rounded-lg px-2 py-1 text-xs text-amber-800 hover:bg-amber-500/10" onClick={dismissTransientRecovery}>知道了</button>
        </div>
      )}

      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
        <div className="flex flex-1 overflow-hidden">
          <div className="flex-shrink-0 border-r border-outline-variant bg-surface-container transition-all duration-200 overflow-hidden" style={{ width: missionPanelCollapsed ? 40 : 224 }}>
            <MissionSidebar />
          </div>

          <div className="flex-shrink-0 border-r border-outline-variant bg-surface-container-low transition-all duration-200 overflow-hidden" style={{ width: listPanelCollapsed ? 40 : 288 }}>
            <NoteListPanel />
          </div>
          ...
          <div className="flex-1 flex overflow-hidden">
            {currentNoteId ? (
              ...
              <div className="flex-1 overflow-auto p-6">
                <h2 className="text-lg font-semibold text-on-surface mb-4">{mission!.title} — 看板总览</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {overviewBoardIds.map((bid) => {
                    const board = boards[bid]
                    if (!board) return null
                    return (
                      <button key={bid} className="p-4 rounded-xl border border-outline-variant bg-surface-container hover:bg-surface-container-high transition-colors text-left" onClick={() => setActiveBoard(bid)}>
                  ...
                  <button className="p-4 rounded-xl border border-dashed border-outline-variant hover:bg-surface-container-high transition-colors text-left" onClick={handleCreateBoard}>
                  ...
                  <button className="p-4 rounded-xl border border-dashed border-outline-variant hover:bg-surface-container-high transition-colors text-left" onClick={handleCreateNote}>
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined text-base text-on-surface-variant">add</span>
              ...
              <div className="flex-1 flex items-center justify-center text-on-surface-variant">
                <div className="text-center space-y-4">
                  ...
                  <div className="flex gap-3 justify-center">
                    <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-dashed border-outline-variant hover:bg-surface-container-high transition-colors" onClick={handleCreateBoard}>
                      <span className="material-symbols-outlined text-base">view_kanban</span>
                      <span className="text-sm">新建看板</span>
                    </button>
                    <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-dashed border-outline-variant hover:bg-surface-container-high transition-colors" onClick={handleCreateNote}>
        ...
        <DragOverlay dropAnimation={null}>
          {activeId && activeType ? <DragOverlayContent type={activeType} id={activeId} /> : null}
        </DragOverlay>
      </DndContext>
    </div>
  )
