...
export const useKanbanStore = create<KanbanStore>()(
  (set, get) => ({
    ...
    setWorkSpace: (id) => {
      set({ activeWorkSpaceId: id, currentMissionId: null, currentNoteId: null, currentBoardId: null, previewMissionId: null })
    },
    deleteWorkSpace: (id) => {
      set((s) => ({
        workspaces: s.workspaces.filter((w) => w.id !== id),
        activeWorkSpaceId: s.activeWorkSpaceId === id ? null : s.activeWorkSpaceId,
        missionOrder: Object.fromEntries(Object.entries(s.missionOrder).filter(([k]) => k !== id)),
      }))
    },
    renameWorkSpace: (id, newName) => {
      set((s) => ({ workspaces: s.workspaces.map((w) => (w.id === id ? { ...w, name: newName } : w)) }))
    },
    createMission: (mission) => {
      set((s) => {
        ...
        return {
          ...
          currentBoardId: null,
          previewMissionId: null,
          missionOrder: wsId ? { ...s.missionOrder, [wsId]: [...prevOrder, mission.id] } : s.missionOrder,
        }
      })
    },
    setMission: (id) => { set({ currentMissionId: id, currentNoteId: null, currentBoardId: null, previewMissionId: null }) },
    setPreviewMission: (id) => { set({ previewMissionId: id }) },
    clearPreviewMission: () => { set({ previewMissionId: null }) },
    deleteMission: (id) => {
      set((s) => {
    ...
    renameMission: (id, newTitle) => { set((s) => ({ missions: { ...s.missions, [id]: { ...s.missions[id], title: newTitle } } })) },
    reorderMissions: (workspaceId, orderedIds) => { set((s) => ({ missionOrder: { ...s.missionOrder, [workspaceId]: orderedIds } })) },
    createBoard: (board) => {
      set((s) => {
        ...
        return {
          ...
          missions: { ...s.missions, [missionId]: { ...mission, boardIds: [...mission.boardIds, board.id] } },
          boardOrder: { ...s.boardOrder, [missionId]: [...(s.boardOrder[missionId] ?? []), board.id] },
        }
      })
    },
    deleteBoard: (id) => {
      set((s) => {
        ...
        for (const mid of Object.keys(updatedMissions)) {
          ...
          if (m.boardIds.includes(id)) updatedMissions[mid] = { ...m, boardIds: m.boardIds.filter((bid) => bid !== id) }
        }
        for (const mid of Object.keys(updatedBoardOrder)) {
          if (updatedBoardOrder[mid]?.includes(id)) updatedBoardOrder[mid] = updatedBoardOrder[mid].filter((bid) => bid !== id)
        }
        return { boards: restBoards, missions: updatedMissions, boardOrder: updatedBoardOrder }
      })
    },
    renameBoard: (id, newTitle) => { set((s) => ({ boards: { ...s.boards, [id]: { ...s.boards[id], title: newTitle } } })) },
    reorderBoards: (missionId, orderedIds) => { set((s) => ({ boardOrder: { ...s.boardOrder, [missionId]: orderedIds } })) },
    createTask: (boardId, task) => {
      set((s) => {
        const board = s.boards[boardId]
        return {
          tasks: { ...s.tasks, [task.id]: task },
          boards: { ...s.boards, [boardId]: { ...board, taskIds: [...board.taskIds, task.id] } },
        }
      })
    },
    deleteTask: (boardId, taskId) => {
      set((s) => {
        ...
        return {
          tasks: restTasks,
          boards: { ...s.boards, [boardId]: { ...board, taskIds: board.taskIds.filter((id) => id !== taskId) } },
        }
      })
    },
    renameTask: (_boardId, taskId, newTitle) => { set((s) => ({ tasks: { ...s.tasks, [taskId]: { ...s.tasks[taskId], title: newTitle } } })) },
    moveTask: (taskId, sourceBoardId, targetBoardId, targetIndex) => {
      set((s) => {
        ...
        if (targetIndex !== undefined) newTargetTaskIds.splice(targetIndex, 0, taskId)
        else newTargetTaskIds.push(taskId)
        return { boards: { ...s.boards, [sourceBoardId]: { ...sourceBoard, taskIds: newSourceTaskIds }, [targetBoardId]: { ...targetBoard, taskIds: newTargetTaskIds } } }
      })
    },
    reorderTasks: (boardId, orderedIds) => { set((s) => ({ boards: { ...s.boards, [boardId]: { ...s.boards[boardId], taskIds: orderedIds } } })) },
    addSubTask: (taskId, subTask) => { set((s) => ({ tasks: { ...s.tasks, [taskId]: { ...s.tasks[taskId], subtasks: [...s.tasks[taskId].subtasks, subTask] } } })) },
    removeSubTask: (taskId, subTaskId) => { set((s) => ({ tasks: { ...s.tasks, [taskId]: { ...s.tasks[taskId], subtasks: s.tasks[taskId].subtasks.filter((st) => st.id !== subTaskId) } } })) },
    toggleSubTask: (taskId, subTaskId) => { set((s) => ({ tasks: { ...s.tasks, [taskId]: { ...s.tasks[taskId], subtasks: s.tasks[taskId].subtasks.map((st) => st.id === subTaskId ? { ...st, done: !st.done } : st) } } })) },
    reorderSubtasks: (taskId, orderedIds) => {
      set((s) => {
        const task = s.tasks[taskId]
        if (!task) return s
        const sorted = orderedIds.map((id) => task.subtasks.find((st) => st.id === id)).filter(Boolean) as SubTask[]
        return { tasks: { ...s.tasks, [taskId]: { ...task, subtasks: sorted } } }
      })
    },
    toggleMissionPanel: () => { set((s) => ({ missionPanelCollapsed: !s.missionPanelCollapsed })) },
    toggleListPanel: () => { set((s) => ({ listPanelCollapsed: !s.listPanelCollapsed })) },
    clearRehydrationError: () => { set({ rehydrationError: null }) },
    applyTransientRecovery: (snapshot, message) => {
      set({ ...snapshot, rehydrationError: null, transientRecoveryActive: true, transientRecoveryMessage: message })
    },
    dismissTransientRecovery: () => { set({ transientRecoveryMessage: null }) },
    setCenterTab: (tab) => { set({ centerTab: tab }) },
    setActiveBoard: (boardId) => { set({ currentBoardId: boardId, currentNoteId: null }) },
    createNote: (missionId, note) => {
      set((s) => ({ notes: { ...s.notes, [note.id]: note }, missions: { ...s.missions, [missionId]: { ...s.missions[missionId], noteIds: [...s.missions[missionId].noteIds, note.id] } } }))
    },
    setActiveNote: (missionId, noteId) => { set({ currentMissionId: missionId, currentNoteId: noteId, currentBoardId: null, previewMissionId: null }) },
    deleteNote: (missionId, noteId) => {
      set((s) => {
        const { [noteId]: _, ...restNotes } = s.notes
        return { notes: restNotes, missions: { ...s.missions, [missionId]: { ...s.missions[missionId], noteIds: s.missions[missionId].noteIds.filter((id) => id !== noteId) } }, currentNoteId: s.currentNoteId === noteId ? null : s.currentNoteId }
      })
    },
    renameNote: (_missionId, noteId, newTitle) => { set((s) => ({ notes: { ...s.notes, [noteId]: { ...s.notes[noteId], title: newTitle } } })) },
    updateNoteBlocks: (noteId, blocks) => { set((s) => ({ notes: { ...s.notes, [noteId]: { ...s.notes[noteId], blocks } } })) },
    insertBlock: (noteId, afterIndex, blockType) => {
      set((s) => {
        const note = s.notes[noteId]
        if (!note) return s
        const newBlock: Block = { id: crypto.randomUUID(), type: blockType, content: '', ...(blockType === 'code' ? { language: 'javascript' } : {}) }
    ...
    reorderNotes: (missionId, orderedIds) => {
      set((s) => {
        const mission = s.missions[missionId]
        if (!mission) return s
        return { missions: { ...s.missions, [missionId]: { ...mission, noteIds: orderedIds } } }
      })
    },
    ...
    linkBlockToTask: (noteId, blockId, link) => {
      set((s) => {
        const note = s.notes[noteId]
        if (!note) return s
        const blocks = note.blocks.map((b) => b.id === blockId ? { ...b, linkedBoardId: link?.boardId, linkedTaskId: link?.taskId, linkedSubTaskId: link?.subTaskId } : b)
        return { notes: { ...s.notes, [noteId]: { ...note, blocks } } }
      })
    },
    getCurrentContext: () => {
      ...
      return { activeWorkSpaceId: s.activeWorkSpaceId, currentMissionId: s.currentMissionId, currentMissionTitle: mission?.title ?? null, currentNoteId: s.currentNoteId, currentNoteTitle: note?.title ?? null, previewMissionId: s.previewMissionId, effectiveMissionId: s.previewMissionId ?? s.currentMissionId }
    },
  }),
)
...
attachSqlitePersist(useKanbanStore, {
  ...
  restore: (persisted) => ({ ...(restorePersistedKanbanState(persisted) as Partial<KanbanStore>), rehydrationError: null, transientRecoveryActive: false, transientRecoveryMessage: null }),
  shouldPersist: (state) => !state.transientRecoveryActive,
  onError: (store, error, phase) => {
    const message = error instanceof Error ? error.message : String(error)
    store.setState({ rehydrationError: phase === 'hydrate' ? `本地工作区数据加载失败：${message}。如需恢复，可重置本地数据后重新从云端拉取。` : `云端工作区数据重新加载失败：${message}。请稍后重试，或重置本地数据后重新拉取。` } as Partial<KanbanStore>)
  },
})
